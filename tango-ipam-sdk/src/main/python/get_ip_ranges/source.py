...
import logging
import json
from vra_ipam_utils.meutils import oputils_range_to_vra
...
def handler(context, inputs):
    logging.debug("Made it to handler")
...
def do_get_ip_ranges(self, auth_credentials, cert):
    logging.info('In handler function')
    # Your implemention goes here
    username = auth_credentials["privateKeyId"]
    password = auth_credentials["privateKey"]
    
    logging.info("Loging dict ---> {0}".format(self.inputs))
    
    props = get_properties(self.inputs)
    apiKey = props["apiKey"]
    params = {"apiKey": apiKey}

    response = requests.get("http://" +  self.inputs["endpoint"]["endpointProperties"]["hostName"] + "/api/json/ipam/getAllSubnetSummary",params)
    ...
    result_ranges = []
    for r in rows:
        translated = oputils_range_to_vra(r)
        result_ranges.append(translated)
