...
def do_validate_endpoint(self, auth_credentials, cert):
    ...
    try:
        ...
        logging.info("endpoint was " + self.inputs["endpointProperties"]["hostName"])
        
        #response = requests.get("http://" + self.inputs["endpointProperties"]["hostName"] + "/apiclient/ember/index.jsp", verify=False, auth=(username, password))
        response = requests.get("http://" +  self.inputs["endpointProperties"]["hostName"] + "/apiclient/ember/index.jsp", verify=False, auth=HTTPBasicAuth(username, password))
