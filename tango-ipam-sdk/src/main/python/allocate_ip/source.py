...
import logging
from vra_ipam_utils.meutils import get_properties
...
def allocate_in_range(range_id, resource, allocation, context, endpoint):
    ...
    #raise Exception("Throwing exception intentionally")
    ...
    props = get_properties(endpoint)
    apiKey = props["apiKey"]
    logging.info(f"APIKEY from props {apiKey}")
    params = {"apiKey": apiKey, "status": '3', "subnetAddress": range_id}

    response = requests.get("http://" +  endpoint["endpointProperties"]["hostName"] + "/api/json/ipam/getUnUsedIP",params)
    
    if response.status_code == 200:
        logging.info("200 response OK getunusedip")
    else:
        raise Exception(f"Non 200 getunusedip response code {response.status_code}")
    
    response_json = response.json()
    rows = response_json['rows']
    logging.info("SHouldnt print")
    
    #Find an Available IP to use.  Ignore those that are reserved.
    found_ip = None
    for r in rows:
        if r['reserved-status'] == 'Not Reserved':
           found_ip = r['ip-address']
           logging.info(f"Found Not Reserved ip {found_ip}")
           break
        else:
           logging.info(f"ip was reserved {r['ip-address']}")
           
    #If we found IP set it as used
    if found_ip is not None:
        set_ip_used(found_ip, endpoint)
    else:
        raise Exception(f"Unable to allocate ip in range {range_id}")

           

    result["ipAddresses"] = [found_ip]

    return result

def set_ip_used(found_ip, endpoint):
    props = get_properties(endpoint)
    apiKey = props["apiKey"]
    params = {"apiKey": apiKey, "markAs": "Used", "IPADDRESS": found_ip}

    response = requests.post("http://" +  endpoint["endpointProperties"]["hostName"] + "/api/json/ipam/updateIPState",params)
    
    if response.status_code == 200:
        logging.info(f"200 response OK marked used {found_ip}")
    else:
        raise Exception(f"Non 200 response code {response.status_code}")
