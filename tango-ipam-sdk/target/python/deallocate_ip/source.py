...
def do_deallocate_ip(self, auth_credentials, cert):
    ...
    hostName = endpoint["endpointProperties"]["hostName"]
    
    for deallocation in self.inputs["ipDeallocations"]:
        deallocation_result.append(deallocate(self.inputs["resourceInfo"], deallocation, apiKey, hostName))
...
def deallocate(resource, deallocation, apiKey, hostName):
    ...
    params = {"apiKey": apiKey, "markAs": "Available", "IPADDRESS": ip}

    response = requests.post("http://" +  hostName + "/api/json/ipam/updateIPState",params)
    
    if response.status_code == 200:
        logging.info(f"200 response OK dealocate succeeded")
    else:
        logging.error(f"Non 200 response code {response.status_code} dealocate failed")
